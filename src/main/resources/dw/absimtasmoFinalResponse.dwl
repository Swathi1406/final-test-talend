...
fun getMOT_APP_ID(r) =
  if (!isEmpty(r.MOT_AP_ID_Licenca)) (r.MOT_AP_ID_Licenca splitBy " ")[0][0 to 3]
  else if (!isEmpty(r.MOT_AP_ID_Ferias))
    if (((r.MOT_AP_ID_Ferias splitBy " ")[0]) == "F") "2072" else r.MOT_AP_ID_Ferias
  else if (!isEmpty(r.MOT_AP_ID_Time_off)) (r.MOT_AP_ID_Time_off splitBy " ")[0][0 to 3]
  else ""
...
toStream(rows)
  filter (
    ($.Quantidade_Calculada_Horas != null and $.Quantidade_Calculada_Horas != 0)
  ...
  map {
    ...
    START_DATE:  if (!isEmpty($.First_Day_of_Leave))
                 ...
                 else (parseAbsenceDate($.Absence_Date) as String {format: "yyyy-MM-dd"}),
